<template>
  ...
  <div v-else-if="!invitationDetails && !loading">
    <div class="text-center text-neutral-500 text-sm">
      Invitation not found or has expired.
    </div>
  </div>
  <div v-else class="bg-white rounded-3xl px-8 py-10 flex flex-col gap-8 invitation-card">
    <!-- Header -->
    ...
    <p class="text-center text-neutral-500 text-sm">
      ...
      <RouterLink :to="{ name: 'Guests.SigninView' }"
...
defineProps<{
  invitationDetails: InvitationDetailsResponseData | null
  loading?: string
}>()
