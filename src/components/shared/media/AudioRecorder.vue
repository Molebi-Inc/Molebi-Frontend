<template>
  <div class="flex flex-col">
    ...
    <div class="flex border-b border-neutral-200 mb-5">
      <button v-for="tab in (['Upload File', 'Record Audio'] as const)" :key="tab"
    ...
    <div v-if="activeTab === 'Upload File'" class="flex flex-col items-center gap-4">
      <div v-if="!uploadedFile">
        <MediaUploadDropzone accept="audio/*" label="Drag audio file here" @files="onAudioFileSelected" />
      </div>
      <div v-else class="w-full">
        <AudioPreviewPlayer :audio-url="audioUrl!" :title="uploadedFile.name" @delete-audio="deleteAudio" />
      </div>
    </div>
    ...
    <div v-else class="flex flex-col items-center gap-5 min-h-[200px]">
      ...
      <div v-if="recordState === 'idle'" class="flex flex-col items-center gap-4 py-4">
        <button
          ...
          <svg width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <path d="M5 13C5 18 9 22 14 22C19 22 23 18 23 13" stroke="#9CA3AF" stroke-width="2"
      ...
      <div v-else-if="recordState === 'recording' || recordState === 'paused'" class="w-full flex flex-col gap-4">
        ...
        <div class="flex items-center justify-center gap-[2px] h-12 px-4">
          <div v-for="i in 48" :key="i" class="w-1 rounded-full bg-neutral-400 transition-all"
      ...
      <div v-else-if="recordState === 'recorded'" class="w-full flex flex-col gap-4">
        ...
        <AudioPreviewPlayer v-if="audioUrl" :audio-url="audioUrl" :title="'Recording'" @delete-audio="deleteAudio" />

        <!-- Extra controls row: speed, download -->
        <!-- <div class="flex items-center justify-center gap-4 text-neutral-500">
          <button
            class="flex items-center gap-1 text-xs font-medium px-2 py-1 rounded-lg hover:bg-neutral-100 transition-colors">
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
              <path d="M7 1V7L10 10" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" />
          ...
          <button class="p-1.5 rounded-lg hover:bg-neutral-100 transition-colors" title="Download">
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
              <path d="M7 1V9M4 6L7 9L10 6M2 12H12" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"
                stroke-linejoin="round" />
            </svg>
          </button>
          <button class="p-1.5 rounded-lg hover:bg-neutral-100 transition-colors text-red-400" title="Delete"
            @click="reset">
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
              <path d="M2 3.5H12M5 3.5V2H9V3.5M5.5 6V10.5M8.5 6V10.5M3 3.5L3.5 12H10.5L11 3.5H3Z" stroke="currentColor"
