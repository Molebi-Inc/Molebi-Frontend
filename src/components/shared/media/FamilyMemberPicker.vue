<template>
  <div class="flex flex-col gap-3">
    <!-- Search -->
    <div class="relative">
      <svg class="absolute left-3 top-1/2 -translate-y-1/2 text-neutral-400" width="15" height="15" viewBox="0 0 15 15" fill="none">
    ...
    <!-- Member list -->
    <div class="flex flex-col divide-y divide-neutral-100 max-h-[240px] overflow-y-auto rounded-xl border border-neutral-100">
      <div v-if="!filteredMembers.length" class="py-6 text-center text-sm text-neutral-400">
        No members found
      </div>
      <button
        ...
        :key="member.id"
        type="button"
        class="flex items-center gap-3 px-3 py-3 text-left transition-colors"
        :class="isSelected(member.id) ? 'bg-primary-50' : 'hover:bg-neutral-50'"
        @click="$emit('toggle', member.id!)"
      >
        <!-- Avatar -->
        <div class="w-9 h-9 rounded-full flex-shrink-0 overflow-hidden bg-neutral-200 flex items-center justify-center">
          <img v-if="member.profile_picture_url" :src="member.profile_picture_url" class="w-full h-full object-cover" />
        ...
        <div class="flex-1 min-w-0">
          <p class="text-sm font-medium text-neutral-900 truncate">{{ member.full_name }}</p>
