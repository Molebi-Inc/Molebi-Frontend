<template>
  <aside class="w-[220px] flex-shrink-0 bg-white rounded-2xl p-3 space-y-1 self-start">
    <!-- All media item -->
    <button
      class="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors"
      :class="
        activeAlbumId === null
    ...
    <div>
      ...
      <div v-if="folders.length" class="space-y-0.5 mt-1">
        <MemoriesAlbumRow
          ...
          :album="album"
          variant="sidebar"
          :class="activeAlbumId === album.id ? 'bg-primary-50!' : ''"
          @select="$emit('select-album', album.id)"
        />
      </div>
