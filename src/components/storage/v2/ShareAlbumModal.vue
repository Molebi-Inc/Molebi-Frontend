<template>
  <MlbModal :show="show" :bottom-sheet="isMobile" :bottom-sheet-height="320"
    ...
    <div class="flex flex-col gap-4 py-1">
      ...
      <!-- Shareable link row -->
      <div class="flex items-start gap-3 p-3 bg-secondary-50 rounded-xl border border-secondary-100">
      ...
      <div v-if="selectedIds.length" class="flex flex-col gap-2">
        <div class="flex items-center justify-between">
          <span class="text-xs font-semibold text-neutral-700">{{ selectedIds.length }} member{{ selectedIds.length !==
            1 ?
            's' : '' }}</span>
          <!-- Permission dropdown -->
          <div class="relative">
            <button
              ...
              {{ permissionLabel }}
              <svg width="12" height="12" viewBox="0 0 12 12" fill="none" :class="showPermission ? 'rotate-180' : ''"
            ...
            <div v-if="showPermission"
              class="absolute right-0 top-full mt-1 bg-white border border-neutral-200 rounded-xl shadow-lg z-10 overflow-hidden w-36">
              <button v-for="opt in permissionOptions" :key="opt.value"
        ...
        <div class="flex flex-col gap-1 max-h-[120px] overflow-y-auto">
          <div v-for="memberId in selectedIds" :key="memberId"
            class="flex items-center gap-2 px-3 py-2 bg-neutral-50 rounded-xl">
            <div
              class="w-7 h-7 rounded-full bg-neutral-200 flex-shrink-0 overflow-hidden flex items-center justify-center">
              <img v-if="getMember(memberId)?.profile_picture_url" :src="getMember(memberId)!.profile_picture_url!"
            ...
            <span class="flex-1 text-xs font-medium text-neutral-800 truncate">{{ getMember(memberId)?.full_name
            }}</span>
      ...
      <div class="flex gap-3">
        <button
          ...
          :disabled="selectedIds.length === 0 || loading" @click="handleShare">
