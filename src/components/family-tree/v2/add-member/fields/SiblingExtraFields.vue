<template>
  <div class="space-y-5">
    ...
    <div>
      ...
      <div class="relative">
        <select :value="sharedParentsValue" placeholder="Select an option"
          ...
          <option v-for="opt in sharedParentsOptions" :key="String(opt.value)" :value="opt.value ?? ''">
            {{ opt.label }}
          </option>
        </select>
    ...
    <!-- Which parent do they share? (shown when not "both" and not "not_sure") -->
    <div v-if="showParentSelect">
      <label class="block text-sm font-medium text-neutral-600 mb-2">
        {{ parentSelectLabel }}
      </label>
      <div class="relative">
        <select :value="modelValue.sibling_parent_id ?? ''"
          class="w-full appearance-none bg-white border border-neutral-200 rounded-xl px-4 py-3 text-sm text-neutral-800 outline-none focus:border-primary-400 transition-colors pr-10"
          @change="onSiblingParentChange">
          <option value="">Select parent</option>
          <option v-for="p in availableParents" :key="p.value" :value="p.value">
            {{ p.label }}
          </option>
        </select>
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"
...
const sharedParentsOptions: { value: V2MemberFormData['shared_parents']; label: string }[] = [
  { value: 'both', label: 'Yes' },
  { value: 'father_only', label: 'Same father only' },
  { value: 'mother_only', label: 'Same mother only' },
...
const onSiblingParentChange = (event: Event) => {
  ...
  update('sibling_parent_id', value ? Number(value) : null)
}

const showParentSelect = computed(
  () =>
    props.modelValue.shared_parents === 'father_only' ||
    props.modelValue.shared_parents === 'mother_only',
)

const parentSelectLabel = computed(() => {
  if (props.modelValue.shared_parents === 'father_only') return 'Select shared father'
  if (props.modelValue.shared_parents === 'mother_only') return 'Select shared mother'
  return 'Select shared parent'
})
